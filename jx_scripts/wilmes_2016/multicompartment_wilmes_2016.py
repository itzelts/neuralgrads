...
# Midpoint path distance from soma center for each branch (used for kap scaling)
# Distances approximated as path from soma(0.5) through tree to branch midpoint
dist_mid = {
    SOMA:       0,
    AIS:        s_diam / 2 + a_diam / 2,                              # 10.25 um
    APROX:      s_diam / 2 + (apical_length / 5) / 2,                 # 59.25 um
    APICAL:     s_diam / 2 + apical_length / 5 + (4 * apical_length / 5) / 2,  # 309.25 um
    OBLIQUE:    s_diam / 2 + apical_length / 5 + d_length / 2,        # 259.25 um
    BASAL_MAIN: s_diam / 2 + (d_length / 2) / 2,                      # 84.25 um
}
for i in BASAL_BR:
    dist_mid[i] = s_diam / 2 + d_length / 2 + (d_length / 2) / 2     # 234.25 um
for i in SECOND_BASAL:
    dist_mid[i] = s_diam / 2 + d_length + (d_length / 2) / 2         # 384.25 um
for i in TUFT:
    dist_mid[i] = s_diam / 2 + apical_length + d_length / 2          # 659.25 um
for i in SECOND_BR:
    dist_mid[i] = s_diam / 2 + apical_length + d_length + d_length / 2  # 959.25 um
...
# kap: distance-dependent scaling (g_KA * (1 + min(dist, 500) / (500/slope)))
# Soma is uniform; non-soma branches scale with distance capped at 500 um
cell.branch(SOMA).set('gbar_kap', g_KA)
for i in NON_SOMA:
    dist = min(dist_mid[i], 500.0)
    gkap = g_KA * (1.0 + dist / (500.0 / slope_KA))
    cell.branch(i).set('gbar_kap', gkap)

# sca: soma × 2, tuft (≈500-750 um zone) × 3, others × 1
cell.set('gbar_sca', gsca)
cell.branch(SOMA).set('gbar_sca', gsca * 2)
for i in TUFT:
    cell.branch(i).set('gbar_sca', gsca * 3)

# kca: soma × 2, others × 1
cell.set('gbar_kca', g_KCa)
cell.branch(SOMA).set('gbar_kca', g_KCa * 2)

# it2: active (git2) in tuft branches (≈500-750 um zone); 0 elsewhere
for i in NON_SOMA:
    if i in TUFT:
        cell.branch(i).set('gbar_it2', git2)
    else:
        cell.branch(i).set('gbar_it2', 0.0)
...
# ── Plot 1: Somatic voltage trace ─────────────────────────────────────────────

fig1, ax1 = plt.subplots(figsize=(10, 4))
ax1.plot(t, v.T[:-1], color='k')
ax1.set_xlim(990, 1100)
ax1.set_ylim(-80, 40)
ax1.set_xlabel('Time (ms)')
ax1.set_ylabel('Voltage (mV)')
ax1.set_title('Somatic Voltage — Wilmes 2016 Multicompartment')
fig1.tight_layout()
fig1.savefig('results/wilmes_2016_multicomp_soma.png', dpi=150)
print("Saved: results/wilmes_2016_multicomp_soma.png")
...
print("Saved: results/wilmes_2016_morphology.png")
